...
generate_reflexive_paradigms_eng($input_filename, $output_filename);
...
sub rule107 {
    ...
    $part1 = substr($radical,0,-3);
    $part2 = substr($radical,-3,3);
    $part2 =~ s/([aʌieɛuoɔɵʉ])t/$1̂j/; # keep an 'i' because it has to be displayed as 'इ'
    $radical = $part1.$part2;
    return $radical;
}
...
sub generation_reflexive_paradigms_eng {
    my ($variable1) = $_[0];
    my ($variable2) = $_[1];
    my $result = "";
...
sub generate_reflexive_paradigms_eng {
    ...
    while (<in_file>) {
        ...
        chomp($data);
        my @words = split('-', $data);
        print out_file generation_reflexive_paradigms_eng($words[0], $words[1]);
    }
    close(in_file);
    close(out_file);
}
